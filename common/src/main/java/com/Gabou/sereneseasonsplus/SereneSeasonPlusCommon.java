...
public class SereneSeasonPlusCommon {
    ...
    protected void onTick(Level level,
                          ...
                          boolean keepFullDayNightCycleAtFixedLength,
                          double fullDayNightCycleLengthInRealMinutes,
        ...
        if (enableSeasonalDaylightCycle) {
            ...
            TimeSpeeds seasonalWeights = seasonalTimeSpeedProvider.get(currentSubSeason);
            if (seasonalWeights == null) {
                LOGGER.warn("No day/night speed config found for {}.", currentSubSeason);
                return;
            }
            if (keepFullDayNightCycleAtFixedLength) {
                ConfigHacks.TimeSpeeds normalized = ConfigHacks.normalizeToCycleMinutes(
                        seasonalWeights.daySpeed(), seasonalWeights.nightSpeed(), fullDayNightCycleLengthInRealMinutes);
                desiredSpeeds = new TimeSpeeds(normalized.daySpeed(), normalized.nightSpeed());
            } else {
                desiredSpeeds = seasonalWeights;
            }
