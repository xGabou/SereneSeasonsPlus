...
public class FabricEnvironmentHelper implements IEnvironmentHelper {
    ...
    public int getGrassChance(boolean force) {
        ...
        switch (season) {
            case EARLY_SUMMER, LATE_SUMMER -> baseChance = 500; // faster
            case MID_SUMMER -> baseChance = 400;   // fastest
            case EARLY_SPRING, LATE_AUTUMN ->  baseChance = 1400; // slowest
            case MID_SPRING, MID_AUTUMN -> baseChance = 1000;    // slower
            case LATE_SPRING, EARLY_AUTUMN -> baseChance = 800;   // slow
        }
